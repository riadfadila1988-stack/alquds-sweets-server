...
type ArgMap = { [k: string]: string | undefined };
...
(async function main(): Promise<void> {
  ...
  try {
    ...
    const WorkDayPlan: any = models && (models.WorkDayPlan || models.default || models);
    ...
    for (let plan = await cursor.next(); plan != null; plan = await cursor.next()) {
      ...
      if (modified) {
        await plan.save();
        plansTouched++;
        if (sample.length < 10) sample.push(String(plan._id));
      }
    }
